...
export default async function getApplicants(internshipId, sortBy, status) {
  try {
    ...
    query.push({
      $lookup: {
        from: "internships", ////which document to jooin, collectiion name not schema
        localField: "internship_id", //which field of this document to compare
        foreignField: "_id", //which field of other document to compare
        as: "internship", // what will be the name of the joined document
      },
    });
