...
export async function getInternshipsWithMatchScore(search, filter) {
  try {
    ...
    const intenshipsBySectors = await Internship.aggregate([
      { $match: query },
      { $sort: { createdAt: -1 } },
      {
        $lookup: {
    ...
    let internshipsWithMatchScore = (
      await Promise.all(
        intenshipsBySectors?.map(async (item) => {
          const parsedRequiredSkills = await getParsedRequiredSkills(item);
          const company_logo = getCompanyLogoUrl(item.company_logo);

          //return if there is not parsed required skills
          if (parsedRequiredSkills?.length === 0) return null;

          const { matchScore, matchedSkills, matchColour } =
            calculateMatchScore(parsedRequiredSkills, parsedSkills);
